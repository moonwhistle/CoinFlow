...
배치 처리는 성능 면에서 정말 압도적이지만, 이로 인해 새로운 고민거리들이 생겼습니다.
...
메시지 처리한 후 한 번에 ACK를 날리기 직전에 컨슈머 애플리케이션이 강제 종료된다면, Redis 입장에서는 500개 모두 아직 처리되지 않은 Pending 상태로 남게 됩니다. 이후 해당 메시지는 다시 컨슈머에게 전달되어 중복 처리될 위험이 있었습니다.



해결1: 최근 처리된 RecordId 기반 필터링 적용
처음에는 중복 처리를 방지하기 위해 Redis Stream ID의 순차성을 이용해 '최근 처리 ID보다 작으면 스킵'하는 방식을 고려했습니다. 하지만 이 방식은 **PEL(Pending Entries List) 복구** 시나리오에서 치명적인 문제를 일으킬 수 있습니다.

*   **문제점 (1-3-2 시나리오)**: 시점상 뒤에 있는 틱(ID 3)이 먼저 처리된 후, 이전에 실패했던 과거 틱(ID 1, 2)이 복구되어 유입될 경우, 단순 대소 비교 로직은 이를 '이미 처리된 과거 데이터'로 오판하여 **실제로 처리된 적 없는 데이터를 누락(Data Loss)**시킬 위험이 있습니다.

이를 해결하기 위해 순서에 의존하지 않는 **'정확한 ID 존재 여부 검증'** 방식을 도입했습니다.
...
#### [기술적 포인트]
1.  **순서 무관성**: 대소 비교가 아닌 **존재 여부(Exact Match)**를 체크하므로, 메시지가 어떤 순서로 재유입되어도 정확히 중복만 걸러낼 수 있습니다.
2.  **LRU(Least Recently Used) 전략**: T2.micro의 제한된 자원을 고려하여, 가장 오래된 기록을 자동으로 밀어내는 LRU 알고리즘을 적용했습니다. 10만 건의 ID를 보관하는 데 약 5~10MB의 메모리만 사용하여 매우 효율적입니다.
3.  **데이터 무결성**: 인메모리 집합 연산 전 단계에서 필터링을 수행하므로, 중복 틱으로 인해 거래량(Volume)이나 체결 횟수가 뻥튀기되는 정합성 문제를 원천 차단했습니다.
