...
4. DB 전용 스레드는 데이터 저장을 수행하며, 저장 파이프라인이 완료된 뒤 `BatchAckWorker`에 ACK 대상 `RecordId`를 넘깁니다.

이렇듯, 비동기 방식으로 변경함으로써 Consumer Thread의 차단을 막아 **실시간성을 확보**함과 동시에, 실제 XACK는 저장 완료 이후 Batch ACK로 처리하여 **데이터 유실 방지(At-least-once 기반 처리)**와 Redis I/O 절감을 함께 가져가는 구조를 완성했습니다.
...
public Executor dbPersistExecutor() {
    ...
    executor.setCorePoolSize(properties.corePoolSize());      // 기본 유지 스레드 수 (현재 설정: 10)
    executor.setMaxPoolSize(properties.maxPoolSize());        // 최대 스레드 수 (현재 설정: 10)
...
이후 현재 설정에서는 DB 저장 파이프라인의 처리량을 더 확보하기 위해 `core-pool-size`와 `max-pool-size`를 각각 10으로 조정했습니다.
