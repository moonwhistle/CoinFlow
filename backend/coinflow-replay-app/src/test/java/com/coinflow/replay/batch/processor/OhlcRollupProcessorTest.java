...
class OhlcRollupProcessorTest {
        ...
        void process_CreateNewOhlc5m() {
                ...
                RollupTarget target = new RollupTarget(mockSymbol, start,
                                ReconciliationBatchConstants.INTERVAL_5M_MINUTES);
        ...
        void process_CreateNewOhlc30m() {
                ...
                LocalDateTime start = LocalDateTime.of(2024, 3, 12, 10, 0);
                RollupTarget target = new RollupTarget(mockSymbol, start,
                                ReconciliationBatchConstants.INTERVAL_30M_MINUTES);
                ...
                when(ohlc1mService.findCandlesInBucketRange(eq(mockSymbol.getId()), eq(start), any()))
                                .thenReturn(sources);
                when(ohlc30mService.findBySymbolIdAndBucketTime(mockSymbol.getId(), start))
                ...
                // then
                assertThat(result).isInstanceOf(Ohlc30m.class);
                assertThat(result.getOpenPrice()).isEqualByComparingTo("100");
        ...
        void process_SkipWhenMatched() {
                ...
                existing.apply(new BigDecimal("100"), new BigDecimal("110"), new BigDecimal("95"),
                                new BigDecimal("105"),
                                1000L);

                when(ohlc1mService.findCandlesInBucketRange(eq(mockSymbol.getId()), eq(start), any()))
                                .thenReturn(sources);
                when(ohlc5mService.findBySymbolIdAndBucketTime(mockSymbol.getId(), start))
