...
class TickProcessServiceTest {
    ...
    void processLateTickTest() {
        ...
        assertAll(
                ...
                () -> verify(dbPersistService, times(1)).persistClosedCandleAsync(eq("btcusdt"),
                        eq(closedKlineSnapshot)),
                // 4. 비동기 작업 종료 후 Redis ACK 확인
                () -> verify(Objects.requireNonNull(redisTemplate.opsForStream()), timeout(1000)).acknowledge("mystream", "mygroup",
                        recordId));
    }
}
